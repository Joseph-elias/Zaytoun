...
from app.services.olive_land_pieces import ensure_land_piece_exists
...
def _to_out(item: FarmerOliveSeason, labor_map: dict, sales_map: dict, usage_map: dict) -> dict:
    ...
    if item.tanks_20l is not None:
        remaining_tanks = _round2(Decimal(item.tanks_20l) - sold_tanks - used_tanks)
...
def create_olive_season(db: Session, farmer_user_id: UUID, payload: OliveSeasonCreate) -> dict:
    ...
    ensure_land_piece_exists(db, farmer_user_id, str(payload.land_piece_name))
    ...
    item = FarmerOliveSeason(
        ...
        pressing_cost=payload.pressing_cost,
        notes=payload.notes,
    )
    db.add(item)
    db.commit()
...
def update_olive_season(db: Session, season_id: UUID, farmer_user_id: UUID, payload: OliveSeasonUpdate) -> dict | None:
    ...
    item.pressing_cost = payload.pressing_cost
